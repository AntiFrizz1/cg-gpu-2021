...
bool PixelShader::Initialize(DeviceComPtr device_ptr, WCHAR const* shader_path, LPCSTR sz_entry_point, LPCSTR sz_shader_model, D3D_SHADER_MACRO* p_defines)
{
	HRESULT hr = S_OK;
	...
	if (FAILED(hr))
	{
		...
		utils::WinErrorLogger::Log(hr, "Failed to compile pixel shader");
		return false;
	}
