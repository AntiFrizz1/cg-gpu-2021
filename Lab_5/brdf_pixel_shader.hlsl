...
float3 Ambient(float3 n, float3 v)
{
    ...
    //float3 r = 2 * myDot(n, v) * n - v;
    float prefiltered_mip_level = roughness * MAX_REFLECTION_LOD;
    //float3 prefilteredColor = prefiltered_color_texture.SampleLevel(g_samPoint, r, prefiltered_mip_level);
    float3 prefilteredColor = prefiltered_color_texture.SampleLevel(samLinear, r, prefiltered_mip_level);
